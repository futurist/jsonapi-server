...
filter._filterMatches = function(textToMatch, propertyText, schema) {
  ...
  if (textToMatch[0] === ">") {
    var gte = textToMatch[1] === "=";
    textToMatch = gte ? castValue(textToMatch, 2) : castValue(textToMatch, 1);
    if (typeof propertyText === "number") textToMatch = parseInt(textToMatch, 10);
    if (gte? textToMatch <= propertyText : textToMatch < propertyText) return true;
  } else if (textToMatch[0] === "<") {
    var lte = textToMatch[1] === "=";
    textToMatch = lte ? castValue(textToMatch, 2) : castValue(textToMatch, 1);
    if (typeof propertyText === "number") textToMatch = parseInt(textToMatch, 10);
    if (lte ? textToMatch >= propertyText : textToMatch > propertyText ) return true;
  } else if (textToMatch[0] === "~") {
